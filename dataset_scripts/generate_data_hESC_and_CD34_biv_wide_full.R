...
res = bfcif(bfc,
            ...
            rname, function(){
                ...
                names(np_gr) = sub("_pool.+", "", names(np_gr))
                olaps_hm = ssvOverlapIntervalSets(np_gr)
                rs = rowSums(as.data.frame(mcols(olaps_hm)))
                olaps_hm = olaps_hm[rs > 1]
                ...
                cn = cn[cn != "id"]
                colnames(overlap_dt) = gsub("\\.", "-", colnames(overlap_dt))
                # colnames(overlap_dt) = gsub("-narrow", ".narrow", colnames(overlap_dt))
                # colnames(overlap_dt) = gsub("-broad", ".broad", colnames(overlap_dt))
                ...
                overlap_dt = overlap_dt[, c("id", cn), with = FALSE]
