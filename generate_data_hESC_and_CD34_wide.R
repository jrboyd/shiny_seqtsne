...
res = bfcif(bfc,
            # force_overwrite = T,
            "hESC_CD34_seqtsne_wide", function(){
    ...
    #resize
    query_gr = resize(olaps_hm, 5000, "center")

    #fetch raw data to establish norm_factor
    options(mc.cores = 32)
    raw_bw = stsFetchTsneInput(cfg_dt, query_gr, cap_value = Inf)$bw_dt
    dt_quant = ssvSignalBandedQuantiles(raw_bw, by_ = c("mark"), return_data = TRUE)
    ...
    list(profile_dt, tsne_dt, query_gr, agg_dt, overlap_dt, annotation_dt, cfg_dt)
    # res
})
